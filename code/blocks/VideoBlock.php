...
class VideoBlock extends Block
{
    ...
    public function getCMSFields()
    {
        ...
        if (is_array($ratios)) {
            ...
            if (count($values) > 0) {
                $ratioField = DropdownField::create('ManualRatio', _t('VideoBlock.ASPECT_RATIO', 'Aspect ratio'),
                    $values);
            }
        }
        ...
        if ($this->MediaThumbID) {
            ...
            $fields->addFieldToTab('Root.Main',
                LiteralField::create('MediaThumb',
                    '<div class="field"><div class="middleColumn">' . $thumb->Tag . '</div></div>')
            );
        }
